...
CREATE TABLE ra_raum (
    ra_ge_name TEXT REFERENCES ge_name,
...
CREATE TABLE do_dose (
    do_nummer TEXT,
    do_ra_nummer INTEGER REFERENCES ra_nummer,
    do_ra_stockwerk INTEGER REFERENCES ra_stockwerk,
    PRIMARY KEY (do_nummer, do_ra_nummer, do_ra_stockwerk)
);

CREATE TABLE sw_switch (
    sw_ip TEXT,
    sw_sc_nummer INTEGER REFERENCES sc_nummer,
    sw_sc_stockwerk INTEGER REFERENCES sc_stockwerk,
    PRIMARY KEY (sw_ip)
);

CREATE TABLE sd_switch_zu_dose (
    sd_sw_ip INTEGER REFERENCES sw_ip,
    sd_do_nummer TEXT REFERENCES do_nummer,
    sd_do_ra_nummer INTEGER REFERENCES do_ra_nummer,
    sd_do_ra_stockwerk INTEGER REFERENCES do_ra_stockwerk,
    sd_switchport TEXT,
