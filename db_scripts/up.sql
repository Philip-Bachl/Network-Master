...
CREATE TABLE ra_raum (
    ...
    ra_ge_name TEXT REFERENCES ge_gebaeude(ge_name) NOT NULL,
...
CREATE TABLE sc_schrank (
    ...
    sc_ge_name TEXT REFERENCES ge_gebaeude(ge_name) NOT NULL,
    PRIMARY KEY (sc_nummer, sc_stockwerk, sc_ge_name)
);

CREATE TABLE do_dose (
    do_nummer TEXT NOT NULL,
    do_ra_ge_name TEXT REFERENCES ra_raum(ra_ge_name) NOT NULL,
    do_ra_nummer INTEGER REFERENCES ra_raum(ra_nummer) NOT NULL,
    do_ra_stockwerk INTEGER REFERENCES ra_raum(ra_stockwerk) NOT NULL,
...
CREATE TABLE sw_switch (
    ...
    sw_sc_ge_name TEXT REFERENCES sc_schrank(sc_ge_name) NOT NULL,
    PRIMARY KEY (sw_ip)
);

CREATE TABLE sd_switch_zu_dose (
    sd_sw_ip TEXT REFERENCES sw_switch(sw_ip) NOT NULL,
    sd_do_nummer TEXT REFERENCES do_dose(do_nummer) NOT NULL,
    sd_do_ra_nummer INTEGER REFERENCES do_dose(do_ra_nummer) NOT NULL,
    sd_do_ra_stockwerk INTEGER REFERENCES do_dose(do_ra_stockwerk) NOT NULL,
    sd_do_ra_ge_name TEXT REFERENCES do_dose(do_ra_ge_name) NOT NULL,
